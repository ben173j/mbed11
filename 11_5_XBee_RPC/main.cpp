...
int main(){
   ...
   xbee.write("ATMY 0x265\r\n", 12);
   reply_messange(xbee_reply, "setting MY : 0x265");
   xbee.write("ATDL 0x165\r\n", 12);
...
//BSP_ACCELERO_AccGetXYZ(initDataXYZ);
      printf("%s\r\n", outbuf);
  //    printf("x: %d , y: %d , z: %d ",initDataXYZ[0],initDataXYZ[1],initDataXYZ[2]);
...
void getAcc(Arguments *in, Reply *out)
{
    ...
    sprintf(buffer, "Accelerometer values: (%d, %d, %d)", initDataXYZ[0]
,initDataXYZ[1],initDataXYZ[2]);
    out->putData(buffer);
}
